...
def run_merging_process(UMparam_files, source_dirs, MAX_C_RATIO=1.0):
    ...
    for UMparam_file, source_dir in zip(UMparam_files, source_dirs):    
        ...
        target_dir = source_dir.replace(
            ...
            r"DeepUM_NatMeth2026V2_merged\merged_data"
        )

        for idx, KSDir in enumerate(data['KS_dirs']):
            print(f"Processing KSDir: {KSDir} (idx {idx})...")
            KSDir = data['KS_dirs'][idx]
            target_KSDir = os.path.join(target_dir, str(idx))
            ...
            for file_name in ['spike_times.npy', 'channel_positions.npy', 'cluster_bc_unitType.tsv', 'cluster_info.tsv']:
            ...
            sess_idx = (mt['RecSes 1'] == idx+1) & (mt['RecSes 2'] == idx+1)
