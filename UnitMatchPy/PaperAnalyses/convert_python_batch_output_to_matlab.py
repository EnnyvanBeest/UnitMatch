...
def convert_python_output_to_matlab(save_dir, output_mat_path=None):
    ...
    if output_mat_path.exists():
        raise FileExistsError(f'{output_mat_path} already exists; refusing to overwrite')
    ...
    matlab_cols = ['ID1', 'ID2', 'RecSes1', 'RecSes2', 'UID1', 'UID2', 'MatchProb', 'TotalScore', 'EucledianDistance']
    for col in matlab_cols:
        if col not in match_table.columns:
            match_table[col] = np.nan
    match_table = match_table[matlab_cols + [c for c in match_table.columns if c not in matlab_cols]]
    ...
    scipy.io.savemat(str(output_mat_path), {
        'clusinfo': clusinfo,
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        raise SystemExit('Usage: convert_python_batch_output_to_matlab.py <save_dir> [output_mat_path]')
    save_dir = sys.argv[1]
