...
def _prepare_session(merged_dir):
    ...
    session_names = sorted(
        (d for d in os.listdir(merged_dir)
    ...
    print(f'  {waveform.shape[0]} units across {param["n_sessions"]} session(s) after merging')
...
def parse_args():
    parser = argparse.ArgumentParser(description='Run DeepUnitMatch and UMPy on the merged dataset.')
...
def main():
    ...
    for i, merged_dir in enumerate(groups):
        ...
        if run_deep:
            ...
            print(f'  Skipping DeepUnitMatch (results exist, REDO=False): {get_save_dir(merged_dir)}')
        ...
        if run_ump:
            ...
            print(f'  Skipping UMPy (results exist, REDO=False): {get_umpy_save_dir(merged_dir)}')
