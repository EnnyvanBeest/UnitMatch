...
def get_parameter_kernels(scores_to_include, labels, cond, param, add_one=1):
    ...
    for sc in scores_to_include:
        ...
        smooth_tmp = (
            smooth_prob  # Not doing the different ones for now (default the same)
        )
        ...
        for ck in range(len(cond)):
            ...
            parameter_kernels[:, score_id, ck] /= np.sum(
                parameter_kernels[:, score_id, ck]
            )
            parameter_kernels[:, score_id, ck] += add_one * np.min(
                parameter_kernels[
                    parameter_kernels[:, score_id, ck] != 0, score_id, ck
                ],
                axis=0,
            )
...
def apply_naive_bayes(parameter_kernels, priors, predictors, param, cond):
    ...
    for start_idx in range(0, n_pairs, chunk_size):
        ...
        for metric_idx in range(n_metrics):
            ...
            distances = np.abs(
                metric_values[:, np.newaxis] - score_vector[np.newaxis, :]
            )
    ...
    for ck in range(len(cond)):
        ...
        tmp_prob = parameter_kernels[
            min_idx, metric_indices, ck
        ]  # (n_pairs, n_metrics)
    ...
    prob = (priors[np.newaxis, :] * likelihood) / denominator[
        :, np.newaxis
    ]  # (n_pairs, len(cond))
