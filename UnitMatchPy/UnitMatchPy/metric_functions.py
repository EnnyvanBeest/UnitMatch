...
def get_Euclidean_dist(avg_waveform_per_tp_flip, param, n=None):
    ...
    data_cv0 = avg_waveform_per_tp_flip[
        :, :, waveidx, 0, :
    ]  # (3, n_units, len(waveidx), n_flips)
    data_cv1 = avg_waveform_per_tp_flip[
        :, :, waveidx, 1, :
    ]  # (3, n_units, len(waveidx), n_flips)
...
def get_euclidean_metrics_chunked(avg_waveform_per_tp_flip, param):
    ...
    for i_start in range(0, n_units, chunk_size):
        ...
        diff = (
            data_cv0[:, i_start:i_end, np.newaxis, :, :]
            - data_cv1[:, np.newaxis, :, :, :]
        )
...
def get_recentered_metrics_chunked(avg_waveform_per_tp_flip, avg_centroid, param):
    ...
    for i_start in range(0, n_units, chunk_size):
        ...
        # (3, chunk, 1, waveidx, flips) - (3, 1, N, waveidx, flips) = (3, chunk, N, waveidx, flips)
        diff = (
            rc_cv0[:, i_start:i_end, np.newaxis, :, :] - rc_cv1[:, np.newaxis, :, :, :]
        )
        # Euclidean norm over spatial dims -> (chunk, N, waveidx, flips)
        ed = np.linalg.norm(diff, axis=0)
        del diff
        ...
        raw_result[i_start:i_end, :] = np.nanmin(np.nanmean(ed, axis=2), axis=-1)
        del ed
...
def dist_angle(avg_waveform_per_tp_flip, param):
    ...
    for dim_id1 in range(avg_waveform_per_tp_flip.shape[0]):
        for dim_id2 in np.arange(1, avg_waveform_per_tp_flip.shape[0]):
            if dim_id2 <= dim_id1:
                continue
            ang = np.abs(x1[dim_id1, :, :, :, :] - x2[dim_id1, :, :, :, :]) / (
                np.abs(x1[dim_id2, :, :, :, :] - x2[dim_id2, :, :, :, :]) + 1e-10
            )
...
def apply_drift_correction_basic(
    ...
    ##need to add the drift to the location
    avg_waveform_per_tp[0, session_switch[sid + 1] : session_switch[sid + 2], :, :] += (
        drift[0]
    )
    avg_waveform_per_tp[1, session_switch[sid + 1] : session_switch[sid + 2], :, :] += (
        drift[1]
    )
    avg_waveform_per_tp[2, session_switch[sid + 1] : session_switch[sid + 2], :, :] += (
        drift[2]
    )
    ...
    if n_shanks != 1:
        for i in range(n_shanks):
            ...
            if np.all(correct_shank_a == correct_shank_b) != True:
                ...
                print(
                    f"These pairs may be bad {bad_idx}, so are excluded from drift correction"
                )
...
def drift_n_sessions(
    ...
    for did in range(n_sessions - 1):
        ...
        if (
            ...
            drift_shank, avg_waveform_per_tp, avg_centroid = (
...
def drift_correct_session_pair(
    candidate_pairs, session_switch, avg_centroid, avg_waveform_per_tp, did, param
):
    """
    This function applies drift correction between n_sessions, currently this is done by aligning session 2 to session 1,
    then session 3 to session 2 etc.
    ...
    if candidate_pairs.shape != (n_units, n_units):
        raise ValueError(
            "candidate_pairs must have shape (n_units, n_units) matching avg_centroid/avg_waveform_per_tp."
        )
    if session_switch.ndim != 1 or session_switch.size < 3:
        raise ValueError(
            "session_switch must be a 1D array with at least 3 entries (start, boundary, end)."
        )
    if session_switch[0] != 0 or session_switch[-1] != n_units:
        raise ValueError(
            "session_switch must be relative to the provided arrays (session_switch[0]==0 and session_switch[-1]==n_units)."
        )
    ...
    if test_matches_per_shank(pairs, avg_centroid, did, param) == True:
        _, avg_waveform_per_tp, _ = apply_drift_correction_per_shank(
            pairs, did, session_switch, avg_centroid, avg_waveform_per_tp, param
        )
        print(
            f"Done drift correction per shank for session pair {did + 1} and {did + 2}"
        ...
        _, avg_waveform_per_tp, _ = apply_drift_correction_basic(
            pairs, did, session_switch, avg_centroid, avg_waveform_per_tp
        )
    elif len(pairs) == 0:
        print("No pairs across sessions to perform drift correction.")
