...
def process_single_location(location_data, path_lookup, param):
    ...
    try:
        ...
        conn, cursor = util.create_robust_db_connection(
            r"C:\Users\suyash\UCL\DeepUnitMatch\DeepMatch\matchtables.db"
        )
        ...
        def _check_if_processed():
            ...
            if "MatchProbNew" in columns:
                ...
                cursor.execute(
                    f"SELECT COUNT(*) FROM {table_name} WHERE MatchProbNew IS NULL"
                )
        ...
        KS_dirs = path_lookup.loc[
            (path_lookup["mouse"] == mouse)
            & (path_lookup["probe"] == probe)
            & (path_lookup["loc"] == loc),
            "recordings",
        ].to_list()[0]
        ...
        waveform, session_id, session_switch, within_session, good_units, param = (
            util.load_good_waveforms(
                wave_paths, unit_label_paths, param, good_units_only=False
            )
        )
        waveform, session_switch, within_session, session_id, good_units, param = (
            util.filter_good_units_and_merge(
                mt_path, mouse, KS_dirs, waveform, session_switch, param
            )
        )
        ...
        mt = pd.read_sql_query(
            f"SELECT RecSes1,RecSes2,ID1,ID2 FROM {table_name}", conn
        )
        ids = {}
