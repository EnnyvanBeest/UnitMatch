...
def merge_split_unit(row, spk_df: pd.DataFrame, data_dir):
    ...
    save_path = os.path.join(
        waveforms_dir,
        f"Unit{str(int(row['ID1']))}+{str(int(row['ID2']))}_RawSpikes.npy",
    )
    new_data = {
...
def C_ratios(within, threshold_dnn, spk_df, null=False, t_r=0.002, t_c=0.0001):
    ...
    for idx, row in putative_splits_1dir.iterrows():
        clus = spk_df.loc[spk_df["recses"] == row["RecSes1"], "clusters"].item()
        times = spk_df.loc[spk_df["recses"] == row["RecSes1"], "times"].item()
        if len(times[np.where((clus == row["ID1"]))]) > len(
            times[np.where((clus == row["ID2"]))]
        ):
...
def merge_and_remove_splits(
    param, prob_matrix, session_id, model, data_dir, max_C_ratio=1
):
    """
    ...
    merges = [
        (idx, df.loc[idx, "uid1"].item(), df.loc[idx, "uid2"].item(), ratio)
        for idx, ratio in match_C_ratios.items()
    ...
    }  # Initialize removals dict for all sessions
    ...
    if len(to_remove) > 0:
        for ses in range(n_sessions):
            if removals[ses]:  # If there are removals in this session
                ...
                if ses == 0:
                    param["good_units"][ses] = np.delete(
                        param["good_units"][ses].squeeze(), removals[ses]
                    )
                else:
                    # Calculate offset for this session
                    offset = session_switch[ses]
                    adjusted_removals = [
                        r - offset for r in removals[ses] if r >= offset
                    ]
                    if adjusted_removals:
                        param["good_units"][ses] = np.delete(
                            param["good_units"][ses].squeeze(), adjusted_removals
                        )
                    else:
    ...
    session_id = np.concatenate(
        [np.full(len(param["good_units"][ses]), ses) for ses in range(n_sessions)]
    ).astype(int)
    new_dnnsim = inference(model, data_dir)

    # Update session_switch based on new good_units lengths
    new_session_switch = np.cumsum(
        [len(param["good_units"][ses]) for ses in range(n_sessions)]
    )
    ...
    for exp in os.listdir(data_dir):
        ...
        for file in os.listdir(waveforms_path):
            if "#" in file:
                ...
                os.rename(
                    os.path.join(waveforms_path, file),
