...
def run(args):
    ...
    np_dataset = AE_NeuropixelsDataset_cortexlab(
        root=np_root, batch_size=args.batchsize
    )
    ...
    train_dataset, val_dataset, test_dataset = torch.utils.data.random_split(
        np_dataset, [train_size, val_size, test_size]
    )
    train_loader = DataLoader(
        train_dataset, batch_size=args.batchsize, shuffle=True, num_workers=4
    )
    val_loader = DataLoader(
        val_dataset, batch_size=args.batchsize, shuffle=True, num_workers=4
    )
...
def run_training(
    ...
    Args:
        exp_name: Experiment name for saving checkpoints and logs
        dataset: pytorch Dataset object for training
        lr: Learning rate (default: 1e-5)
        save_freq: Frequency of saving model (default: 1)
    ...
    save_folder = os.path.join(save_path, "ModelExp", "AE_experiments", args.exp_name)
    if os.path.exists(save_folder) and not args.cont:
        # Check if the folder already exists and if we are not continuing training
        raise ValueError(
            "Running this experiment will overwrite a previous one. Please choose a different name."
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "--lr", "-l", type=float, default=1e-5, help="Learning rate"
    )
    arg_parser.add_argument(
    ...
    arg_parser.add_argument(
        "--batchsize", "-b", type=int, default=32, help="batch size"
    )
    arg_parser.add_argument(
        "--train_root",
        type=str,
