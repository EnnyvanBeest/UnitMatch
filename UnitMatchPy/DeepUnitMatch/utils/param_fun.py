...
def sort_good_channels(goodChannelMap, goodpos, n_xchannelpos=2):
    ...
    if n_unique_x != n_xchannelpos * n_shanks:
        print(
            f"Found {n_unique_x} unique x-axis positions across {n_shanks} shank(s); "
...
def extract_Rwaveforms(waveform, ChannelPos, ChannelMap, param):
    ...
    waveform = waveform[
        start_time:end_time, :, :
    ]  # selecting the middle 60 time points (11-71)
    ...
    Rwaveform = (
        Rwaveform - GlobalMean
    )  # subtracting the global mean, zero mean is good for DNN
    ...
    if z_MaxSitepos < mean_z_sorted_goodpos:
        ...
        pad_after = (
            padding_needed  # Pad at the end if MaxSitepos is above the mean z position
        )
    Rwaveform = np.pad(
        Rwaveform,
...
def get_snippets(
    waveforms, ChannelPos, session_id, save_path=None, unit_ids=None, param=None
):
    """
    ...
    if processed_root.exists():
        try:
            ...
            print(
                f"Warning: could not remove temporary directory {processed_root}: {e}"
            )
    ...
    for i, unit in enumerate(waveforms):
        ...
        if (
            ...
            or n_channels != expected_n_channels
            or n_repeats != 2
        ):
            print(
                f"Expected waveform shape to be ({expected_n_time},{expected_n_channels},2) - instead got {unit.shape}"
            )
            continue
        if np.any(np.isnan(unit)) or np.any(np.isinf(unit)):
            if not np.any(np.isnan(unit[:, :, 0])) and not np.any(
                np.isinf(unit[:, :, 0])
            ):
                unit = unit[:, :, 0]
                (
    ...
    return (
        np.array(processed_waveforms),
        np.array(positions),
        np.array(kept_indices, dtype=int),
    )
