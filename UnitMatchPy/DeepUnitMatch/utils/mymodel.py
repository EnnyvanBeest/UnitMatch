...
class Conv1DSeq(nn.Module):
    ...
    def __init__(
        self, n_channels, n_filters, kernel_sizes, dilation_rates, expected_seq_len
    ):
        """
...
class SpatioTemporalCNN_V2(nn.Module):
    def __init__(self, n_channel, n_time, n_output=256):
        ...
        self.ConvBlock2 = Conv1DSeq(
            self.n_time, [self.n_time, self.n_time], [3, 3], [1, 2], self.n_channel
        )
...
class Decoder_SpatioTemporalCNN_V2(nn.Module):
    def __init__(self, n_channel, n_time, n_input=256):
        ...
        self.DeConvSpatialBlock2 = nn.Sequential(
            nn.ConvTranspose1d(
                self.n_channel, self.n_channel, kernel_size=3, padding=1
            ),
            nn.LayerNorm([self.n_channel, self.n_time]),
            nn.GELU(),
        )
        self.DeConvTimeLayer = nn.ConvTranspose1d(
            self.n_time, self.n_time, kernel_size=3, padding=1
        )
        self.__init_weight()
...
class SpatioTemporalAutoEncoder_V2(nn.Module):
    def __init__(self, n_channel, n_time, n_output=256):
        ...
        self.decoder = Decoder_SpatioTemporalCNN_V2(
            self.n_channel, self.n_time, self.n_output
        )
