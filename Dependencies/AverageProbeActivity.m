...
%             subksdirs(cell2mat(cellfun(@(X) any(strfind(X,'Chronic')),{subksdirs(:).folder},'UniformOutput',0))) = []; %Use separate days and match units via matlab script
%         else
%             disp('Using chronic pyks option')
%             subksdirs = subksdirs(cell2mat(cellfun(@(X) any(strfind(X,'Chronic')),{subksdirs(:).folder},'UniformOutput',0))); %Use chronic output from pyks
%         end
        ...
        for probeid = 1:length(ProbeOpt)
            ...
            % Match Units and assign unique clusID
            MatchUnitsAlgorithm
