...
function error = plot_venn3( a, b, c, d, e, f, g, resolution, dist_A_B, dist_B_C, dist_A_C )
    ...
    img(:,:,1) = img(:,:,1) + 2 .* ( (X - center1_x).^2 + (Y - center1_y).^2 < r1^2 );
    img(:,:,2) = img(:,:,2) + 4 .* ( (X - center2_x).^2 + (Y - center2_y).^2 < r2^2 );
    img(:,:,3) = img(:,:,3) + 6 .* ( (X - center3_x).^2 + (Y - center3_y).^2 < r3^2 );
    img(repmat(all(img==0,3),[1,1,3]))=1;
%     clf
    imagesc(img)
    hold on
