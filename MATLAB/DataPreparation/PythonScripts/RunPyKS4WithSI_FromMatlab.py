...
def run_ks4_si(bin_file):
    ...
    if imec_match is None:
        print(
            f"Warning: could not parse imec index from {bin_file.name}; defaulting to {stream_base}"
        )
    ...
    recording = se.read_spikeglx(
        bin_file.parent, stream_name=stream_name, load_sync_channel=False
    )
    ...
    recording = spre.phase_shift(
        recording
    )  # correct for time delay between recording channels
    ...
    sorting_ks4 = ss.run_sorter(
        "kilosort4", recording, output_dir, remove_existing_folder=True
    )
